#if UNITY_EDITOR
namespace _EditorTools.SelectionHistory {
    ...
    using PS = UnityEditor.PlayerSettings;

    [InitializeOnLoad]
    public class SelectionHistoryWindow : EditorWindow {
        private const string EditorPrefsKeyPath        = "PinnedAssetPath";
        private const string EditorPrefsKeyPinnedCount = "PinnedAssetCount";
        ...
        public static void Init() {
            ...
            string iconKey = EditorGUIUtility.isProSkin ? "d_UnityEditor.SceneHierarchyWindow" : "UnityEditor.SceneHierarchyWindow";
            window.titleContent.image = EditorGUIUtility.IconContent(iconKey).image;
        ...
        public static bool RecordHierarchy {
            get => EditorPrefs.GetBool($"{PS.productName}_SH_RecordHierarchy", true);
            set => EditorPrefs.SetBool(PS.productName + "_SH_RecordHierarchy", value);
        }

        public static bool RecordProject {
            get => EditorPrefs.GetBool($"{PS.productName}_SH_RecordProject", true);
            set => EditorPrefs.SetBool($"{PS.productName}_SH_RecordProject", value);
        }

        public static bool WorkInBackground {
            get => EditorPrefs.GetBool($"{PS.productName}_SH_WorkInBackground", false);
            set => EditorPrefs.SetBool($"{PS.productName}_SH_WorkInBackground", value);
        }

        public static int MaxHistorySize {
            get => EditorPrefs.GetInt(PS.productName + "_SH_MaxHistorySize", 50);
            set => EditorPrefs.SetInt(PS.productName + "_SH_MaxHistorySize", value);
        }
        ...
        private static void OnUnityClose() {
            SavePinnedToPrefs();
        }
        ...
        private void DrawHeader() {
            ...
            var  backIcon     = EditorGUIUtility.IconContent(IconPrefix + "back@2x").image;
            var  forwardIcon  = EditorGUIUtility.IconContent(IconPrefix + "forward@2x").image;
            var  trashIcon    = EditorGUIUtility.IconContent(IconPrefix + "TreeEditor.Trash").image;
            var  settingsIcon = EditorGUIUtility.IconContent(IconPrefix + "Settings").image;
        ...
        private void DrawHistoryItems(List<(int index, Object item)> items, string actionButtonLabel, System.Action<Object> actionButtonAction, List<Object> baseList) {
            ...
            foreach (var tuple in items) {
                ...
                if (GUILayout.Button(buttonContent, EditorStyles.label, GUILayout.MaxHeight(buttonHeight)))
                ...
                if (GUILayout.Button(actionButtonLabel, GUILayout.Width(buttonWidth)))
