...
namespace InfimaGames
{
    ...
    public class Spring
    {
        ...
        public Vector3 Evaluate()
        {
            ...
            float m = Mathf.Max(settings.mass, 0.0001f);
            ...
            float _stepSize = deltaTime > stepSize ? stepSize : Mathf.Max(deltaTime - 0.001f, 0.0001f);
            float steps = Mathf.Ceil(deltaTime / _stepSize);
            if (steps <= 0f || float.IsNaN(steps) || float.IsInfinity(steps))
                return currentValue;
