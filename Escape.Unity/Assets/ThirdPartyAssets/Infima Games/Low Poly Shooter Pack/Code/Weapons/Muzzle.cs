...
namespace InfimaGames.LowPolyShooterPack
{
    ...
    public class Muzzle : MuzzleBehaviour
    {
        ...
        [Tooltip("Firing VFX")]
        [SerializeField]
        private GameObject prefabFlashVfx;

        [Tooltip("local offset applied to the VFX")]
        [SerializeField]
        [ShowIf(nameof(prefabFlashVfx))]
        private Vector3 flashVfxOffset;

        [Tooltip("Local scale applied to the VFX")]
        [SerializeField]
        [ShowIf(nameof(prefabFlashVfx))]
        private Vector3 flashVfxScale = Vector3.one;
        ...
        [SerializeField]
        [ShowIf(nameof(prefabFlashParticles))]
        ...
        [SerializeField]
        [ShowIf(nameof(prefabFlashLight))]
        ...
        private void Awake()
        {
            if (this.prefabFlashVfx != null) {
                this.vfx = Instantiate(this.prefabFlashVfx, socket);
                LayersUtil.SetLayerRecursively(this.vfx.transform, this.gameObject.layer);
                this.vfx.transform.SetLocalPositionAndRotation(this.flashVfxOffset, Quaternion.identity);
                this.vfx.transform.localScale = this.flashVfxScale;
            ...
            if(prefabFlashParticles != null)
            {
                ...
                LayersUtil.SetLayerRecursively(spawnedParticlesPrefab.transform, this.gameObject.layer);
                //Reset the position.
            ...
            if (prefabFlashLight)
            {
                ...
                LayersUtil.SetLayerRecursively(spawnedFlashLightPrefab.transform, this.gameObject.layer);
                //Reset the position.
        ...
        private void OnDrawGizmosSelected() {
            if (this.socket && this.prefabFlashVfx) {
                var src = this.socket.TransformPoint(this.flashVfxOffset);
                Gizmos.DrawWireSphere(src, 0.01f);
                Gizmos.DrawLine(src, src + this.socket.forward * 0.05f);
            }
        }
