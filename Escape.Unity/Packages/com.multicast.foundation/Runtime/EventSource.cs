namespace Multicast {
    ...
    public class EventSource {
        private readonly List<(Action handler, HandlerFlags flags)> handlers = new();

        private uint invocationCount;
        private bool needsCleanup;

        [PublicAPI]
        public void Listen(Lifetime lifetime, [NotNull] Action handler) {
            if (handler == null) {
        ...
        [PublicAPI]
        public UniTask WaitOne(Lifetime lifetime) {
            if (lifetime.IsDisposed) {
        ...
        private Action RegisterInternal(Action handler, HandlerFlags flag) {
        ...
        private bool UnregisterInternal(Action handler) {
            var found = false;
