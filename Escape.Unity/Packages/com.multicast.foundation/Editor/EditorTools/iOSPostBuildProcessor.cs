...
namespace Multicast.EditorTools {
    ...
    public class iOSPostBuildProcessor : ScriptableObject {
        ...
        public static void FixPodFile(BuildTarget buildTarget, string path) {
            var podFile = path + "/Podfile";
            
            var allLines = File.ReadAllLines(podFile).ToList();

            var position = allLines.IndexOf("target 'UnityFramework' do");
            
            allLines.Insert(position + 1, "  use_frameworks!" );
            
            File.WriteAllLines(podFile, allLines.ToArray());
        }
#endif
    }
    ...
    class PostBuildProcessingSettings {
        ...
        private void AddBuildProperties(BuildProperties prop) {
            ...
            foreach (var property in prop.properties){
                foreach (var flagProperty in this.flags[idx].properties){
                    if(flagProperty.Name == property.Name) {
                        exists = true;
                        break;
                    }
                }
                if(!exists)    this.flags[idx].properties.Add(property);
                exists = false;
            }
        }
...
#region Helper Definition
    static class Tools {
        public static void Process(PostBuildProcessingSettings settings, string path) {
            ...
            var targetFramework = project.GetUnityMainTargetGuid();
            ...
            AddFrameworks(project, targetFramework, settings.frameworks);
            foreach(var flag in settings.flags){
                var trg = string.Empty;
                switch (flag.target){
                    case TargetEnum.UnityIphone:
                        trg = project.GetUnityMainTargetGuid();
                        break;
                    case TargetEnum.UnityIphoneTests:
                        trg = project.TargetGuidByName(PBXProject.GetUnityTestTargetName());
                        break;
                    case TargetEnum.UnityFramework:
                        trg = project.GetUnityFrameworkTargetGuid();
                        break;
                    case TargetEnum.Project:
                        trg = project.ProjectGuid();
                        break;
                    default:
                        trg = project.TargetGuidByName(trg);
                        break;
                }
                AddProperty(project, trg, flag.properties);
            }
        ...
        /// </summary>
        public static void AddProperty(PBXProject proj, string targetGUID, List<BuildProperty> properties) {
