namespace Multicast.ExpressionParser {
    ...
    public class IntParserTest {
        ...
        public int Parse(string input) => Execute(input, null);

        private static int Execute(string input, ExpressionContext<int> context) {
            return ExpressionParserInt.Instance.Compile(input, context, false).Invoke();
        }
    }
}