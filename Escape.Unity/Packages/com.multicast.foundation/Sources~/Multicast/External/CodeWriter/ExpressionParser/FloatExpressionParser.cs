...
namespace CodeWriter.ExpressionParser {
    public class FloatExpressionParser : ExpressionParser<float> {
        ...
        protected override float Pow(float a, float b) => (float)Math.Pow(a, b);
        protected override float Equal(float a, float b) => Math.Abs(b - a) < 0.00001f ? 1 : 0;
        protected override float NotEqual(float a, float b) => 1 - Equal(a, b);
        ...
        protected override bool IsTrue(float v) => Math.Abs(v) < 0.00001f;
