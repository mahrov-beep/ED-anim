...
namespace CodeWriter.ExpressionParser {
    ...
    public abstract class ExpressionParser<T> {
        private readonly Dictionary<string, ExprBuilder> builderCached = new Dictionary<string, ExprBuilder>();
        private Parser<ExprBuilder> parserCached;
        ...
        public Expression<T> Compile(string input, ExpressionContext<T> context, bool cache) {
            ...
            parserCached = parserCached ?? CreateParser();

            ExprBuilder builder;
            try {
                if (cache) {
                    if (!builderCached.TryGetValue(input, out builder)) {
                        builder = parserCached.Parse(input);
                        builderCached.Add(input, builder);
                    }
                }
                else {
                    builder = parserCached.Parse(input);
